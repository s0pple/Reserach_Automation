...
class QwenController:
    """
    Abstrahiert die UI von Qwen (https://chat.qwen.ai/).
    """
    ...
    async def init_session(self):
        print("[QwenController] Navigiere zu Qwen...")
    ...
    async def send_prompt(self, prompt: str):
        print("[QwenController] Sende Prompt...")
        try:
            ...
            try:
                await self.page.locator('button:has(svg), div[role="button"]').last.click(timeout=3000)
            except:
    ...
    async def wait_for_response(self, timeout_sec: int = 90) -> str:
        print("[QwenController] Warte auf Antwort...")
        await self.page.wait_for_timeout(3000)
        ...
        while (asyncio.get_event_loop().time() - start_time) < timeout_sec:
            try:
                responses = await self.page.locator('div.markdown-body, [class*="message"]').all()
                if responses:
        ...
        print("[QwenController] Extrahiere Text...")
        assistant_messages = await self.page.locator('div.markdown-body, [class*="message"]').all()
