...
class GeminiController:
    """
    Abstrahiert die UI von Google Gemini (https://gemini.google.com).
    """
    ...
    async def init_session(self):
        print("[GeminiController] Navigiere zu Gemini...")
        await self.page.goto(self.url, wait_until="domcontentloaded")
        await self.page.wait_for_timeout(3000)
        
        try:
            print("[GeminiController] Pruefe auf Popups...")
    ...
    async def send_prompt(self, prompt: str):
        print("[GeminiController] Sende Prompt...")
        try:
            # Gemini nutzt oft einen rich-text Editor
            prompt_box = self.page.locator('[contenteditable="true"], textarea').first
            ...
            try:
                print("[GeminiController] Klicke Sende-Button...")
                await self.page.locator('button[aria-label*="Send message"], button:has(svg)').last.click(timeout=3000)
    ...
    async def wait_for_response(self, timeout_sec: int = 90) -> str:
        print("[GeminiController] Warte auf Antwort...")
        await self.page.wait_for_timeout(3000)
        ...
        while (asyncio.get_event_loop().time() - start_time) < timeout_sec:
            try:
                responses = await self.page.locator('message-content, .message-content').all()
                if responses:
        ...
        print("[GeminiController] Extrahiere Text...")
        assistant_messages = await self.page.locator('message-content, .message-content').all()
