...
class KimiController:
    """
    Abstrahiert die UI von Kimi (https://kimi.moonshot.cn/ oder https://www.kimi.com/).
    """
    ...
    async def init_session(self):
        print("[KimiController] Navigiere zu Kimi...")
    ...
    async def send_prompt(self, prompt: str):
        print("[KimiController] Sende Prompt...")
        try:
            prompt_box = self.page.locator('div[contenteditable="true"], textarea').first
            ...
            try:
                await self.page.locator('button:has(svg), div[role="button"]:has(svg)').last.click(timeout=3000)
            except:
    ...
    async def wait_for_response(self, timeout_sec: int = 90) -> str:
        print("[KimiController] Warte auf Antwort...")
        await self.page.wait_for_timeout(3000)
        ...
        print("[KimiController] Extrahiere Text...")
