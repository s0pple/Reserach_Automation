...
class AIStudioController:
    ...
    async def set_model(self, model_name: str):
        ...
        try:
            # Absolute Koordinate für Fallback auf 1280x720: (1120, 130) ist meist der Dropdown Button
    ...
    async def wait_for_response(self, timeout_sec: int = 60) -> str:
        ...
        # Antwort extrahieren: Wir kopieren aus dem letzten Model-Block!
        print("[AIStudioController] Extrahiere Text...")
        md_viewers = await self.page.locator("ms-text-chunk").all()
        if not md_viewers:
            print("[AIStudioController] Warnung: Kein ms-text-chunk gefunden! Dumping HTML fuer Diagnose...")
        ...
        result = await md_viewers[-1].inner_text()
        return result.strip()
