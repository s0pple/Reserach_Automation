...
class GrokController:
    """
    Abstrahiert die UI von Grok (https://grok.com/).
    """
    ...
    async def init_session(self):
        print("[GrokController] Navigiere zu Grok...")
    ...
    async def send_prompt(self, prompt: str):
        print("[GrokController] Sende Prompt...")
        try:
            ...
            try:
                await self.page.locator('button[aria-label*="Send"], button:has(svg)').last.click(timeout=3000)
            except:
    ...
    async def wait_for_response(self, timeout_sec: int = 90) -> str:
        print("[GrokController] Warte auf Antwort...")
        await self.page.wait_for_timeout(3000)
        ...
        while (asyncio.get_event_loop().time() - start_time) < timeout_sec:
            try:
                responses = await self.page.locator('div.markdown, div.prose, [class*="message"]').all()
                if responses:
        ...
        print("[GrokController] Extrahiere Text...")
        assistant_messages = await self.page.locator('div.markdown, div.prose, [class*="message"]').all()
